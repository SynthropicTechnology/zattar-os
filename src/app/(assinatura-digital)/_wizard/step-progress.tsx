...
function StepProgressVertical({ steps, currentIndex, onRestart, resumeHint }: BaseProps) {
  return (
            ...
            <li key={step.id} className="flex items-center gap-3">
              <span
                className={cn(
                  ...
                  isFuture && 'border-border/50 bg-transparent text-muted-foreground',
                )}
...
function StepProgressHorizontal({ steps, currentIndex, onRestart, resumeHint }: BaseProps) {
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {onRestart && (
          <button
            type="button"
            onClick={onRestart}
            className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-foreground active:scale-95"
            aria-label="Recomeçar"
          >
            <RotateCcw className="h-3.5 w-3.5" />
          </button>
...
export const StepProgress = {
  Vertical: StepProgressVertical,
  Horizontal: StepProgressHorizontal,
}
