...
test.describe('Assinatura Digital - Fluxo Público de Assinatura', () => {
  ...
  test('deve renderizar a tela de welcome e tratar token inválido', async ({ page }) => {
    ...
    await expect(page.getByRole('main').getByText(/Erro ao carregar/i)).toBeVisible();
    await expect(page.getByRole('main').getByText(/link.*inválido ou expirado/i)).toBeVisible();
  });
  ...
  test('deve renderizar a tela de welcome com token simulado na API', async ({ page }) => {
    ...
    await page.route('**/api/assinatura-digital/public/fake-valid-token', async (route) => {
      // Ignorar rotas filhas como /identificacao
    ...
    // Mock da API para salvar identificação
    ...
    // Review Step
    await expect(page.getByText('Revisar Documento')).toBeVisible();
  });
});
